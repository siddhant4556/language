...
invoking one of the static methods \code{spawnUri} or \code{spawn} defined in the \code{Isolate} class in the library \code{dart:isolate}.
...
or \code{clamp} on non-\code{Never} subtypes of \code{num}
...
\LMHash{}%
If $i$ is an invocation of the form \code{$e$.remainder($e_2$)}
then let $C$ is the context type of $i$.
If both $C$ and $T$ are non-\code{Never} subtypes of \code{num},
then:
\begin{itemize}
  \item{} If $C$ is a subtype of \code{double}
      and $T$ is not a subtype of \code{double},
      then the context type of $e_2$ is \code{double}.
  \item{} If both $C$ and $T$ are subtypes of \code{int}
     then the context type of $e_2$ is \code{int}.
  \item{} Otherwise the context type of $e_2$ is \code{num}.
\end{itemize}
Let further $S$ is the static type of $e_2$.
If $T$ is a non-\code{Never} subtype of \code{num}
and $S$ is assignable to \code{num},
then the static type of $i$ is determined as follows:
\begin{itemize}
  \item{} If $T\,\,<:\,\,\code{double}$
    then the static type of $i$ is $T$.
  \item{} Otherwise, if $S$ is a non-\code{Never} subtype of \code{double}
    then the static type of $i$ is \code{double}.
  \item{} Otherwise, if $S$ is a non-\code{Never} subtype of
    either \code{int} or $T$
    then the static type of $i$ is $T$.
  \item{} Otherwise the static type of $i$ is \code{num}.
\end{itemize}

\LMHash{}%
If $i$ is an invocation of the form \code{$e$.clamp($e_2$, $e_3$)}
then let $C$ be the context type of $i$.
If both $C$ and $T$ are non-\code{Never} subtypes of \code{num},
then the context type of $e_2$ and $e_3$ is $C$.
Let further $T_2$ be the static type of $e_2$ and $T_3$ be the static type
of $e_3$.
If $T_2$ and $T_3$ are both non-\code{Never} subtypes of \code{num},
then let $R$ be \LUB(T, T_2, T_3).
If $R\,\,<:\,\,\code{num}$ then the static type of $i$ is $R$,
otherwise the static type of $i$ is \code{num}.
...
%% TODO: Is it necessary to disallow prefix objects.
%% This smells like a leftover from "prefix objects".
...
of the form \code{$e_0$..$s$} or the form \code{$e_0$?..$s$}
...
Otherwise, let \code{v} be a fresh variable bound to $o$ and
evaluate \code{v.$s$} to an object which is discarded, and
...
class \code{int}, \code{dobule} and \code{num}
...
If both $T$ and $C$ are non-\code{Never} subtypes of \code{num}, then
\begin{itemize}
  \item{} If $C$ is a subtype of \code{double}
      and $T$ is not a subtype of \code{double},
      then the context type of $e_2$ is \code{double}.
...
If $T$ is a non-\code{Never} subtype of \code{num}
and $S$ is assignable to \code{num},
then the following rules are used for the static type of $e$:
\begin{itemize}
  \item{} If $T\,\,<:\,\,\code{double}$
...
If the run-time type of $o$ is a subtype of \code{Future<$T$>}, then let $f$ be $o$; otherwise let $f$ be the result of creating a new object using the constructor \code{Future<$T$>.value()} with $o$ as its argument.
